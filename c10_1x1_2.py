...
def block(width,x,stride=1,name=None):
    """Implementation of the B_1x1(2) block.
    ...
    act = Conv2D(width,3,kernel_initializer=init,kernel_regularizer=l2(decay),padding='same',activation='relu',strides=stride,name="{}_act1".format(name))(x)
    act = Dropout(drop)(act)
    act = Conv2D(width,3,kernel_initializer=init,kernel_regularizer=l2(decay),padding='same',activation='relu',name="{}_act2".format(name))(act)
    x = Add()([lin,act])
    x = Dropout(drop)(x)
    return x
