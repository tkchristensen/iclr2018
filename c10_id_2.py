...
drop  = 0.1     # dropout
decay = 0.0002  # l2 regularization (weight decay)
scale = 0.7     # scale for weight initialization
...
def block(width,x,stride=1,name=None):
    """Implementation of the B_id(2) block.
    ...
    lin = x
    if stride > 1:
        lin = Conv2D(width,1,kernel_initializer=init,kernel_regularizer=l2(decay),padding='same',strides=stride,name='{}_lin'.format(name))(lin)
    act = Conv2D(width,3,kernel_initializer=init,kernel_regularizer=l2(decay),padding='same',activation='relu',strides=stride,name='{}a'.format(name))(x)
    act = Dropout(drop)(act)
    act = Conv2D(width,3,kernel_initializer=init,kernel_regularizer=l2(decay),padding='same',activation='relu',name='{}b'.format(name))(act)
    x = Add()([lin,act])
    x = Dropout(drop)(x)
    return x
...
output_class = Dense(10, activation='softmax', name='output')(x)
