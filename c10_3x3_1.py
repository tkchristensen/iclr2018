...
def block(width,x,stride=1,name=None):
    """Implementation of the B_3x3(1) block.
    ...
    lin = Conv2D(width,3,kernel_initializer=init,kernel_regularizer=l2(decay),padding='same',strides=stride, name=name+'_lin')(x)
    act = Conv2D(width,3,kernel_initializer=init,kernel_regularizer=l2(decay),padding='same',activation='relu',strides=stride, name=name+'_act')(x)
    x = Add()([lin,act])
    x = Dropout(drop)(x)
    return x
